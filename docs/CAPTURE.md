...
> **Status in this fork: preparation, not current practice.** Flow Bridge still
> runs the REST transport against `aisandbox-pa.googleapis.com` with a sniffed
> `Bearer ya29.…`, and that still works here. Upstream
> ([crisng95/flowkit](https://github.com/crisng95/flowkit)) migrated to
> batchexecute in September 2026 on the grounds that Google stopped minting that
> token in the rewritten `flow.google.com` frontend. Keep this document so the
> capture workflow is on hand the day the old path stops answering — see
> `/fk-doctor` for how to recognise that day. The upstream path does not yet
> cover video upscale, r2v, or start+end-frame chaining, which is why this fork
> has not moved.
